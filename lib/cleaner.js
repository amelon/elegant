...
.methods({
  clean (data, rulesHash) {
    if (!isObject(data)) return Promise.reject(new VError('data is not an object'));
  ...
  cleanField(value, path, mappedRules) {
    ...
    return Promise.try(function() {
      ...
      var newValue =  R.reduce(function(accuValue, fieldRule) {
        ...
        if (errors.length) {
          let err = R.reduce((accu, error) => new VError(accu, error), new Error('field cleaning error'), errors);
